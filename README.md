...
### input
Stores input data files, obtained from the [Open VB data portal](https://data.vbgov.com/). Users must upload three data files, which should be saved as .xslx: *EMS_Calls_For_Service*, *Fire_Calls_For_Service*, and *Police_Calls_For_Service*. This repository includes the *EMS_Calls_For_Service* obtained through a FOIA request. It also contains *Fire_Calls_For_Service* data set, which was downloaded from the Open VB data portal. Both files cover the period from January 1, 2017 until June 30, 2019. Users need to download the *Police_Calls_For_Service* data set directly from the Open VB data portal and upload the .xlsx file covering the period from January 1, 2017 until June 30, 2019 to the input folder. These files are processed using the SAS program, which generates the OHCA occurrences for a given time period. Details of the analysis can be found in the associated paper (Custodio and Lejeune, 2020).
